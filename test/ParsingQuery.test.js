...
describe('ParsingQuery', () => {
  it('is a constructor', () => {
    strictEqual(typeof ParsingQuery, 'function')
  })
  ...
  describe('.construct', () => {
    it('returns a QuadStream of all quads from the CONSTRUCT query', async () => {
      await withEndpoint(async endpoint => {
        ...
        const query = new ParsingQuery({ endpoint, factory: rdf })
        await endpoint.importFiles([data.triples.path])

        const results = await query.construct('CONSTRUCT { ?s ?p ?o. } WHERE { ?s ?p ?o. }')
  ...
  describe('.select', () => {
    it('returns a Stream of the results as RDF/JS objects', async () => {
      await withEndpoint({ factory: rdf }, async endpoint => {
        ...
        const results = await query.select('SELECT * WHERE { ?s ?p ?o. }')
